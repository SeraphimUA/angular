...
export class ColorPickerComponent implements OnInit {
  ...
  ngOnInit(): void {
    // console.log(this.cssElement?.classList);
    // this.cssElement?.setAttribute('href', 'dark.scss');
  }
  ...
  setTheme(nColor: string, contrast = 'dark') {
    ...
    if (contrast === 'light') {
      ...
      document.documentElement.style.setProperty('--main-text-lightness', '20%');
    } else {
      document.documentElement.style.setProperty('--main-bg-lightness', '20%');
      document.documentElement.style.setProperty('--main-text-lightness', '60%');
    }
  }
